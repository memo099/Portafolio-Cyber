# Portafolio-Cyber
# 🛡️ Portafolio de Ciberseguridad — Memo

## 👤 Sobre mí  

Soy estudiante de Ingeniería en Inteligencia de Datos y Ciberseguridad con interés en seguridad defensiva, redes y concientización digital. Actualmente estoy desarrollando experiencia práctica implementando medidas básicas de ciberseguridad en pequeños despachos y negocios de confianza, enfocándome en prevención de riesgos, protección de información y educación en buenas prácticas digitales.  

---

## 🔐 Servicios que ofrezco  

### ✅ Seguridad de Red y Wi-Fi  
- Configuración segura de routers  
- Cambio de contraseñas de administración  
- Activación de cifrado WPA2/WPA3  
- Creación de red Wi-Fi de invitados  
- Separación de red de trabajo y red personal  

### 🔑 Gestión de Contraseñas  
- Implementación de gestor de contraseñas  
- Generación de contraseñas seguras  
- Eliminación de prácticas riesgosas (misma contraseña en todo)  
- Capacitación básica al personal  

### 💾 Respaldos de Información  
- Configuración de backups automáticos  
- Copias en nube (Google Drive / OneDrive) o disco externo  
- Política básica de respaldo para despachos legales  

### 🎣 Concientización Anti-Phishing  
- Simulacros de correos de phishing  
- Capacitación para identificar amenazas  
- Guía práctica de señales de alerta  

---

## 📌 Proyectos y casos prácticos  

### Caso 1 — Despacho jurídico pequeño  
**Problema:** Red sin contraseña fuerte y sin respaldo de archivos.  
**Solución:** Configuré red segura, red de invitados y backups automáticos.  
**Resultado:** Red más segura y menor riesgo de pérdida de información.  

### Caso 2 — Oficina familiar  
**Problema:** Uso de contraseñas repetidas en múltiples cuentas.  
**Solución:** Implementé gestor de contraseñas y brindé capacitación básica.  

*(Más adelante podrás sustituir estos ejemplos por casos reales con tus tíos o clientes.)*  

---

## 🧠 Habilidades técnicas  

- Fundamentos de ciberseguridad  
- Redes básicas y seguridad Wi-Fi  
- Buenas prácticas de contraseñas  
- Backups y protección de datos  
- Concientización en phishing  
- Linux básico  
- Análisis básico de riesgos  

---

## 🎓 Formación
